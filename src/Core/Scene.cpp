...
Scene::Scene(): QObject(0), m_gizmo(0), m_camera(0) {
    ...
    /// </summary>
    /// 
    m_camera = new Camera(eSceneCamera,this);
    //m_camera->SetCameraType(eSceneCamera);
    m_camera->setObjectName("SceneCamera");
    m_gridlineNameCounter = 1;
    m_cameraNameCounter = 1;
...
Scene::Scene(const Scene & scene): QObject(0) {
    ...
    m_cameraNameCounter = scene.m_cameraNameCounter;
    ...
    for (int i = 0; i < scene.m_perspectCameras.size(); i++)
        addPerspectCamera(new Camera(*scene.m_perspectCameras[i]));
...
//m_perspectCameras
bool Scene::addPerspectCamera(Camera* camera) {
    if (!camera)
        return false;
    if (camera->GetCameraType() == eSceneCamera)
        return false;
    if (m_perspectCameras.contains(camera))
        return false;

    if (camera->GetCameraType() == eSceneCamera)return false;
    m_perspectCameras.push_back(camera);

    camera->setParent(this);
    camera->setObjectName("Camera" + QString::number(m_cameraNameCounter++));
    cameraAdded(camera);

    if (log_level >= LOG_LEVEL_INFO)
        dout << "Camera" << camera->objectName() << "is added to scene" << this->objectName();
...
bool Scene::removeCamera(QObject* c)
{
    for (int i = 0; i < m_perspectCameras.size(); i++)
        if (m_perspectCameras[i] == c) {
            m_perspectCameras.erase(m_perspectCameras.begin() + i);
            cameraRemoved(c);
            if (log_level >= LOG_LEVEL_INFO)
                dout << "Camera" << c->objectName() << "is removed from scene" << this->objectName();
            return true;
        }
    return false;
...
void Scene::childEvent(QChildEvent * e) {
    if (e->added()) {
        ...
        if (removeCamera(e->child()))return;
    }
}
