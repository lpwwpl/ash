...
Model::Model(QObject * parent): AbstractEntity(0), m_type(eM_MESH){
...
Model::Model(const Model & model): AbstractEntity(model) {
    ...
    m_matiral_name = model.m_matiral_name;
    m_joint_transmat = model.m_joint_transmat;
    m_scaling = model.m_scaling;
    m_parent = model.m_parent;
    m_axis = model.m_axis;
    m_type = model.m_type;
    m_vis_transmat = model.m_vis_transmat;
