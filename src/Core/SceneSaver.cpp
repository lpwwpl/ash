...
bool SceneSaver::saveToFile(QString filePath) {
    ...
    out << m_scene->m_perspectCameras.size();
    for (int i = 0; i < m_scene->m_perspectCameras.size(); i++)
        savePerspectCamera(m_scene->m_perspectCameras[i], out);
...
void SceneSaver::savePerspectCamera(Camera* camera, QDataStream& out)
{
    out << camera->movingSpeed();
    out << camera->fieldOfView();
    out << camera->aspectRatio();
