...
Camera::Camera(ECameraType type,QObject* parent): QObject(0) {
...
Camera::Camera(QVector3D position, QVector3D direction, ECameraType type,QObject* parent): QObject(0) {
    setObjectName("Camera");
    setMovingSpeed(0.1f);
...
Camera::Camera(const Camera & camera): QObject(0) {
    ...
    m_up = camera.m_up;
    m_isEnable = camera.m_isEnable;
    m_c_type = camera.m_c_type;
