#include "../include/tinyxml/txml.h"
...
namespace urdf{
	...
	std::shared_ptr<UMaterial> UMaterial::fromXml(TiXmlElement *xml, bool only_name_is_ok) {
		bool has_rgb = false;
		bool has_filename = false;

		std::shared_ptr<UMaterial> m = std::make_shared<UMaterial>();
		...
		if (name_str != NULL) {
			...
			error_msg << "Error! UMaterial without a name attribute detected!";
  ...
  Inertial Inertial::fromXml(TiXmlElement *xml) {
	...
	std::shared_ptr<Visual> Visual::fromXml(TiXmlElement *xml) {
		...
		if (mat != nullptr) {
			...
			vis->material = UMaterial::fromXml(mat, true);
		}
