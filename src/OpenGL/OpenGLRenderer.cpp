...
void OpenGLRenderer::renderNotAxis(OpenGLScene* openGLScene) {
    glClearColor(0.7f, 0.7f, 0.7f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
QOpenGLShaderProgram * OpenGLRenderer::loadShaderFromFile(
    ...
    //m_glFuncs = QOpenGLContext::currentContext()->versionFunctions<QOpenGLFunctions_3_3_Core>();
    //GLuint indx = m_glFuncs->glGetUniformBlockIndex(shader->programId(), "CameraInfo");
    //m_glFuncs->glUniformBlockBinding(shader->programId(), indx, CAMERA_INFO_BINDING_POINT);
    //indx = m_glFuncs->glGetUniformBlockIndex(shader->programId(), "ModelInfo");
    //m_glFuncs->glUniformBlockBinding(shader->programId(), indx, MODEL_INFO_BINDING_POINT);
    //indx = m_glFuncs->glGetUniformBlockIndex(shader->programId(), "MaterialInfo");
    //m_glFuncs->glUniformBlockBinding(shader->programId(), indx, MATERIAL_INFO_BINDING_POINT);
    //indx = m_glFuncs->glGetUniformBlockIndex(shader->programId(), "LightInfo");
    //m_glFuncs->glUniformBlockBinding(shader->programId(), indx, LIGHT_INFO_BINDING_POINT);
    return shader;
}
